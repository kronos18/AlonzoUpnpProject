...
module.exports = function (webServer) {
    ...
    webServer.app.post('/getDescription'
        , getDescription = function (req, res) {
            var brickId = req.body.brickId
                ...
                ;
            if (!brick) {
                console.error("/getDescription : unknown brick", brickId);
            }
            res.json(json);
        }
    );
    ...
    webServer.app.post('/getContext'
        , getContext = function (req, res) {
            ...
            if (node) {
                try {
                    var context = node.getContextDescription()
                    ...
                    console.error("ERROR: getContextDescription", err, node);
                    json = JSON.stringify({});
                }
    ...
    webServer.app.post('/Start'
        , function (req, res) {
            if (req.body.programId) {
                ...
                if (obj) {
                    console.log("Start program", obj.id);
                    obj.Start();
                    console.log("\tstart done...");
                } else {
                    console.error("No program corresponding to id", req.body.programId);
                }
            } else {
                console.log("Start : no program specified");
            }
            res.end();
        }
    );
    webServer.app.post('/Stop'
        , function (req, res) {
            if (req.body.programId) {
                ...
                console.log("Stop  : no program specified");
            }
            res.end();
        }
    );
    ...
    webServer.oncall = function (json, fctCB) {
        ...
        if (obj) {
            ...
            console.error('No object identified by ', json.objectId);
